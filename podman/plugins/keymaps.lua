...
return {
    {
        ...
        opts = function()
            ...
            -----------------------------------------------------------------
            -- Focus Recovery System (for modal interfaces)
            -----------------------------------------------------------------
            _G.focus_stack = _G.focus_stack or {}

            local function push_focus()
                table.insert(_G.focus_stack, {
            ...
            local function pop_focus()
                local focus = table.remove(_G.focus_stack)
                if focus and vim.api.nvim_win_is_valid(focus.win) then
                    vim.api.nvim_set_current_win(focus.win)
                    pcall(vim.api.nvim_win_set_cursor, 0, focus.cursor)
                end
            end

            -- Command palette with focus recovery
            map("n", "<C-S-p>", function()
                push_focus()
                vim.cmd('Telescope commands')
            end, opts("Command Palette"))

            -- Accessibility: Announce buffer changes (subtle, non-intrusive)
            vim.api.nvim_create_autocmd("BufEnter", {
                callback = function()
                    local buf_name = vim.api.nvim_buf_get_name(0)
                    if buf_name ~= "" then
                        local filename = vim.fn.fnamemodify(buf_name, ":t")
                        -- Echo briefly for screen readers, auto-clears
                        vim.api.nvim_echo({{"Focused: " .. filename, "None"}}, false, {})
                    end
                end
            })
            ...
            -- Note: <C-S-p> is defined above with focus recovery
            ...
            -----------------------------------------------------------------
            -- Debugging (DAP) with Focus Management
            -----------------------------------------------------------------
            ...
            map("n", "<leader>du", function()
                ...
                vim.defer_fn(function() vim.cmd('wincmd h') end, 100)
            end, opts("Toggle & Focus Debug UI"))
            -- Cycle through debug panels
            map("n", "<leader>dn", function()
                ...
                for _, win in ipairs(wins) do
                    ...
                    if ft:match('^dap') then
                        table.insert(debug_wins, win)
                    end
                end
                if #debug_wins > 0 then
                    for i, win in ipairs(debug_wins) do
                        if win == current and debug_wins[i + 1] then
                            vim.api.nvim_set_current_win(debug_wins[i + 1])
                            return
                        end
                    end
                    vim.api.nvim_set_current_win(debug_wins[1])
                end
            end, opts("Next Debug Panel"))
