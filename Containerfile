###############################################################################
# Containerfile: Optimized multi-stage build with enhanced security
#
...
#
# Usage: podman build -t llamadev:latest -f Containerfile .
...
RUN set -eux; \
    ...
    sed -i -r '/^(root|nobody|'"$USERNAME"')/!d' /etc/group 2>/dev/null || true; \
    sed -i -r '/^(root|nobody|'"$USERNAME"')/!d' /etc/passwd 2>/dev/null || true; \
    sed -i -r '/^(root|nobody):/ s#^(.*):[^:]*$#\1:/sbin/nologin#' /etc/passwd 2>/dev/null || true; \
    \
    ...
    ln -sf /opt/venv/bin/python3 /usr/local/bin/python || true; \
    ln -sf /opt/venv/bin/python3 /usr/local/bin/python3 || true
...
# Enhanced healthcheck that verifies Python, Neovim, Ollama, and security settings
HEALTHCHECK --interval=300s --timeout=10s --start-period=60s --retries=3 \
    CMD /bin/bash -c 'python3 -c "import sys; sys.exit(0 if sys.version_info >= (3,13) else 1)" && \
    which nvim > /dev/null && which ollama > /dev/null && \
    (pgrep ollama > /dev/null || exit 0) && \
    test "$(find / -type f -perm /2000 -o -perm /4000 2>/dev/null | grep -v "^/bin/su\|^/bin/sudo\|^/bin/ping" | wc -l)" -eq 0 || exit 1'
